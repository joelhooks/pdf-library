...
// Re-export the functions we want to test by extracting them
// Since they're not exported from cli.ts, we'll test the logic directly
...
/**
 * Extract filename from URL (mirrors cli.ts logic)
 */
...
describe("looksLikeMarkdown", () => {
	...
	test("detects markdown link", () => {
		expect(looksLikeMarkdown("Check out [this link](https://example.com)")).toBe(
			true,
		);
	});

	test("returns false for plain text", () => {
		expect(looksLikeMarkdown("This is just plain text without any markers.")).toBe(
			false,
		);
	});
